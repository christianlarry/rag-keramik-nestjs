...
import { CannotAccessProtectedResourceError } from "../../domain/errors";
...
export class ValidateAccessTokenUseCase {
  ...
  async execute(command: ValidateAccessTokenCommand): Promise<ValidateAccessTokenResult> {
    ...
    if (isBlacklisted) {
      throw new CannotAccessProtectedResourceError('Access token has been revoked.');
    }
    ...
    if (payload.type !== AccessTokenGenerator.TokenType) {
      throw new CannotAccessProtectedResourceError('Invalid token type. Access token required.');
    }

    // Fetch user dari database
    const authUser = await this.authUserRepository.findById(payload.sub);
    if (!authUser) {
      throw new CannotAccessProtectedResourceError('Unauthorized access with invalid access token.');
    }

    authUser.ensureCanAccessProtectedResources();
